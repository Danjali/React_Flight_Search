...
describe("<FlightData/>", () => {
  ...
  describe("component rendering based on oneWayflight or connecting flight", () => {
    ...
    it("verifies if flighDetailsLink is rendered for connecting flights", () => {
      ...
      wrapper.find(".detailsAnchor").at(0).props().onClick(e);
    ...
    it("verifies if flightPrice is rendered", () => {
      expect(wrapper.find(".price").at(0).props().children).toEqual([
        "₹ ",
        "3000",
      ]);
      // for connecting flights
      expect(wrapper.find(".price").at(1).props().children).toEqual([
        "₹ ",
        "7000",
      ]);
      expect(wrapper.find(".price").at(2).props().children).toEqual([
        "₹ ",
        "8000",
      ]);
    });
